...
struct SwarmConfig3D {
    ...
    double maxForce = 14.0;
    // Scales the target's orbital motion for demonstration scenarios.
    double targetMotionSpeedMultiplier = 2.5;
...
public:
    ...
    const std::vector<DroneCommState3D>& commStates() const { return m_commStates; }
    void queueCommunicationMessage(const DroneMessage3D& message);
