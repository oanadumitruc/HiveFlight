...
private:
  HiveflightSimNode() : Node("hiveflight_sim_node") {
    ...
    this->declare_parameter("drone_count",          20);
    this->declare_parameter("target_count",          1);
  ...
  void post_init() {
    ...
    cfg.maxForce     = this->get_parameter("max_force").as_double();
    cfg.targetMotionSpeedMultiplier =
      this->get_parameter("target_speed_multiplier").as_double();
    ...
    const int tickMs = std::max(1, static_cast<int>(this->get_parameter("tick_ms").as_int()));
    // The physics timestep must match the configured ROS timer period.
    cfg.dt = static_cast<double>(tickMs) / 1000.0;
    ...
    m_assignment_pub = this->create_publisher<std_msgs::msg::Int32MultiArray>(
      topicPrefix + "/target_assignments", visualizationQos);

    m_targetIndexByDrone.assign(cfg.droneCount, 0);
  ...
  void tick() {
    ...
    std_msgs::msg::Int32MultiArray assignments;
    assignments.data.reserve(m_sim->drones().size());
    for (std::size_t index = 0; index < m_sim->drones().size(); ++index) {
      assignments.data.push_back(static_cast<int32_t>(m_sim->assignedTarget(index)));
    }
    m_assignment_pub->publish(assignments);
    ...
    if (m_sim->tick() % 60 == 0) {
      ...
      const Vec3 target = m_sim->targets().empty() ? Vec3() : m_sim->targets().front();
      const Vec3 drone = drones.empty() ? Vec3() : drones.front().position;
      const double distance = (target - drone).magnitude();
      RCLCPP_INFO(this->get_logger(),
        "tick=%d t=%.2fs alive=%zu/%zu comm_msgs=%zu comm=%s "
        "target0=(%.2f,%.2f,%.2f) drone0=(%.2f,%.2f,%.2f) dist=%.2f",
        m_sim->tick(),
        m_sim->time(),
        alive,
        drones.size(),
